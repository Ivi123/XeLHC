...
try
    delayguess = 0;
    [I_Gas_Sharp,I_Gas_Broad,I_Dissolved,K_Gas,K_Dissolved] = Reconstruct.gx_recon(mrd_files.dixon{1},delayguess,true);
    % delayright = 0;
    % delayguess = 0;
    % while delayright == 0
    % 
...
%Gas Exchange
try
    SNR_Gas_Sharp = QC.basic_snr(I_Gas_Sharp(:,:,:,1),'Gas Exchange - Sharp Gas');
    SNR_Gas_Broad = QC.basic_snr(I_Gas_Broad(:,:,:,1),'Gas Exchange - Broad Gas');
    SNR_Dissolved = QC.basic_snr(I_Dissolved(:,:,:,1),'Gas Exchange - Dissolved');
catch
